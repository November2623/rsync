...
def check_option(args, filesrc, filedest):
    if os.path.isfile(filesrc):
        if args.checksum:
            ...
            return check_size(filesrc, filedest) and\
                   check_time(filesrc, filedest)
    else:
        return False
...
def file_scr_normal(filesrc, filedest):
    if os.path.exists(filedest):
        if os.path.isfile(filedest):
            ...
            copy_content(filesrc, filedest + '/' + filesrc.split('/')[-1])
        else:
            if filedest[-1] == '/':
                os.mkdir(filedest[:-1])
    ...
    try:
        ...
        if os.path.exists(filedest):
            ...
            if os.lstat(filesrc).st_nlink > 1:
                ...
                if filedest[-1] == '/':
                    os.mkdir(filedest[:-1])
                    copy_content(filesrc, os.path.join(filedest, filesrc))
                    # keep_permission(filesrc, os.path.join(filedest, filesrc))
                    # keep_time(filesrc, os.path.join(filedest, filesrc))
                else:
                    copy_content(filesrc, filedest)
        if os.path.isfile(filedest):
        ...
        if a == 0:
            print('rsync: send_files failed to open "' +
                  os.path.abspath(filesrc)+'" : Permission denied (13)')
        else:
            print('rsync: send_files failed to open "' +
                  os.path.abspath(filedest)+'" : Permission denied (13)')
